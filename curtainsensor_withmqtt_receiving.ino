...
char g_curtain1_mqtt_topic[50];        // MQTT topic for reporting curtain1
const char* mqtt_user = "eric";
const char* mqtt_password = "eric";
const char* mqtt_device_id = "Curtain_Sensor1";
...
void setup() {   
  ...
  sprintf(g_curtain1_mqtt_topic, "tele/%x/CURTAIN1",  ESP.getChipId());
...
// State initial curtain position
  if (rx == "Status") //normal operation
  {
       vInp13 = digitalRead(sensorPin);
       if (vInp13 == LOW)
         {    
            client.publish(g_curtain1_mqtt_topic, "1");
            Serial.println("TX: CurtainOpened");
         }
      else
         {
            client.publish(g_curtain1_mqtt_topic, "0");
            Serial.println("TX: CurtainClosed");
        }        
  }
...
void reconnect() {
  ...
  // Attempt to connect
  if (client.connect(g_curtain1_mqtt_topic,mqtt_user,mqtt_password)) {
  ...
  //if (client.connect(mqtt_device_id, mqtt_user, mqtt_password, mqtt_topic, mqtt_qos, mqtt_retain, mqtt_lwt)) {
    Serial.println("connected");
      ...
      // ... and resubscribe
      client.subscribe(g_curtain1_mqtt_topic);
      client.subscribe("command/curtain1");
    } else {
